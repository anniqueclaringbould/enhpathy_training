...
```{r answer_q2_code, eval=F, echo=F}
dim(grn)[1]
...
```{r answer_q5_code, echo=T, eval=T}
...
table_random[fill in a number between 1 and 20] #this shows you one of the tables
```

```{r answer_q7_code, eval=F, echo=F}
nr_links <- lapply(1:rep, function(i){
  sum(table_random[[i]]$n) 
}) %>% unlist() %>% table()
```

```{r answer_q7, eval=F, echo=F}
...
```{r answer_q8_code, eval=F, echo=T}
...
```{r answer_q9_code, eval=F, echo=F}
